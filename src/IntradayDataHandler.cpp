...
DataAggregates IntradayDataHandler::calculate_aggregates(const std::string &underlying, const float &strike, const float &range) {
    DataAggregates agg;
...
void IntradayDataHandler::update_last_fetch() {
...
std::vector<ContractVolumes> IntradayDataHandler::get_volume_par(const std::vector<Contract> &contracts)
{
    ...
    IntradayDataHandler::update_last_fetch();
